...
namespace net.openstack.Core.Providers.v3
{
    using System;
    using net.openstack.Core.Caching;
    using net.openstack.Core.Domain.v3;
    using net.openstack.Providers.Rackspace.v3;
    ...
    /// <threadsafety/>
    /// <preliminary/>
    public class OpenStackIdentityProvider : net.openstack.Providers.Rackspace.v3.CloudIdentityProvider
    {
        /// <summary>
        ...
        public override UserAccess GetUserAccess(CloudIdentity identity, bool forceCacheRefresh = false)
        {
            ...
            Func<UserAccess> refreshCallback =
                () =>
                {
                    ...
                    //    new JProperty("auth", new JProperty(
                    //        new JProperty("identity", new JObject(
                    //            new JArray("methods", JValue.CreateString("password")),
                    ...
                    var response = ExecuteRESTRequest<JObject>(identity, new Uri(UrlBase, "/v3/auth/tokens"), HttpMethod.POST, requestBody, isTokenRequest: true);
                    ...
                    var rawJson = response.Data?.ToString(Formatting.None);
                    if (rawJson == null)
                        return null;
                    ...
                    var firstOrDefault = response.Headers.FirstOrDefault(x => x.Key == "X-Subject-Token");
                    if (firstOrDefault != null)
                        access.Token.Id = firstOrDefault.Value;
