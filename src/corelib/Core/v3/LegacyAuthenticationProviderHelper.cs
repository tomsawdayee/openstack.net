...
using net.openstack.Core.Providers.v3;
using OpenStack.Authentication;
using UserAccess = net.openstack.Core.Domain.v3.UserAccess;

namespace OpenStack.Core.v3
{
    /// <summary>
    ...
    internal static class LegacyAuthenticationProviderHelper
    {
        ...
        {
            net.openstack.Core.Domain.v3.ServiceCatalog service = userAccess.Token.ServiceCatalog.FirstOrDefault(sc => string.Equals(sc.Type, serviceType, StringComparison.OrdinalIgnoreCase));
            if (service == null)
                throw new UserAuthenticationException("The user does not have access to the {0} service or it does not exist.", serviceType);

            net.openstack.Core.Domain.v3.Endpoint endpoint = service.Endpoints
