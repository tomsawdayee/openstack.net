...
using IIdentityService = net.openstack.Core.Providers.v3.IIdentityService;
using Thread = System.Threading.Thread;
using IdentityToken = net.openstack.Core.Domain.v3.IdentityToken;
using Endpoint = net.openstack.Core.Domain.v3.Endpoint;
using ServiceCatalog = net.openstack.Core.Domain.v3.ServiceCatalog;

namespace net.openstack.Providers.Rackspace.v3
{
    /// <summary>
    ...
    {
        /// <summary>
        /// The <see cref="Core.Providers.IIdentityProvider"/> to use for authenticating requests to this provider.
        /// </summary>
        protected readonly Core.Providers.v3.IIdentityProvider IdentityProvider;
        ...
        protected ProviderBase(CloudIdentity defaultIdentity, string defaultRegion, Core.Providers.v3.IIdentityProvider identityProvider, IRestService restService)
        ...
        protected ProviderBase(CloudIdentity defaultIdentity, string defaultRegion, Core.Providers.v3.IIdentityProvider identityProvider, IRestService restService, IHttpResponseCodeValidator httpStatusCodeValidator)
        {
            DefaultIdentity = defaultIdentity;
            _defaultRegion = defaultRegion;
            IdentityProvider = identityProvider ?? this as Core.Providers.v3.IIdentityProvider ?? new CloudIdentityProvider(defaultIdentity);
        ...
        protected Endpoint GetServiceEndpoint(CloudIdentity identity, string serviceType, string serviceName, string region)
        {
            ...
            if (userAccess == null || userAccess.Token.ServiceCatalog == null)
                throw new UserAuthenticationException("Unable to authenticate user and retrieve authorized service endpoints.");

            IEnumerable<ServiceCatalog> services = userAccess.Token.ServiceCatalog.Where(sc => string.Equals(sc.Type, serviceType, StringComparison.OrdinalIgnoreCase));
            ...
            if (string.IsNullOrEmpty(effectiveRegion))
            {
                ...
                else if (!string.IsNullOrEmpty(userAccess.Token.User.DefaultRegion))
                    effectiveRegion = userAccess.Token.User.DefaultRegion;
            }
        ...
        Core.Providers.v3.IIdentityProvider IRackspaceProvider.IdentityProvider
        {
